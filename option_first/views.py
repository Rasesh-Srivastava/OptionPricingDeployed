...
def calc_sum(a,n,p):
    ...
    if(a>n):
        ...
        for j in range(a,n+1):
            ...
            summation = summation + (math.comb(n,j) * (math.pow(p,j)) * (math.pow(1-p,n-j)))
...
def black(request):
    ...
    bs_C = bs_call(S,K,T,r,sigma)
    bs_P = bs_put(S,K,T,r,sigma) #- calc_PutOptionPrice(S,K,r,T,bs_C)
    return render(request, 'option_first/black.html',{'title': 'Rasesh','S': S,'T': T,'K': K,'r': r*100,'sigma': sigma,'bs_C':bs_C,'bs_P':bs_P})
...
def twoStep(request):
    ...
    C = calc_TwoStep_calloptionPrice(p,r,T,Cuu,Cud,Cdd)
    P = calc_PutOptionPrice(S,K,r,T,C)
    return render(request, 'option_first/twoStep.html',{'title': 'Rasesh','S': S,'T': T,'K': K,'r': r*100,'sigma': sigma,'u': u,'d': d,'p': p,'Cuu':Cuu,'Cdd':Cdd,'Cud':Cud,'C':C,'P':P})
...
def nStep(request):
    ...
    n_C = S*calc_sum(a,n,p_dash) - K*math.exp(-1*r*(T))*calc_sum(a,n,p_n)
    n_P = calc_PutOptionPrice(S,K,r,T,n_C)
    return render(request, 'option_first/nStep.html',{'title': 'Rasesh','S': S,'T': T,'K': K,'r': r*100,'sigma': sigma,'n_C':n_C,'n_P':n_P,'n':n,'u_n':u_n,'d_n':d_n,'a':a,'p_n':p_n,'p_dash':p_dash})
